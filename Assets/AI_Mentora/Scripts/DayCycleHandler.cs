...
namespace AIMentora
{
    ...
    public class DayCycleHandler : MonoBehaviour
    {
        ...
        {
            // If time is frozen, set all lights to bright white (full daylight)
            if (GameManager.Instance != null && GameManager.Instance.FreezeTime)
            {
                Color brightWhite = Color.white;
                DayLight.color = brightWhite;
                NightLight.color = Color.clear;  // Turn off night light

#if UNITY_EDITOR
                if(AmbientLight != null)
#endif
                    AmbientLight.color = brightWhite;

#if UNITY_EDITOR
                if(SunRimLight != null)
#endif
                    SunRimLight.color = brightWhite;

#if UNITY_EDITOR
                if(MoonRimLight != null)
#endif
                    MoonRimLight.color = Color.clear;  // Turn off moon light

                LightsRoot.rotation = Quaternion.Euler(0, 0, 180f);  // Sun at top
                UpdateShadow(0.5f);  // Noon shadows
                return;
            }
