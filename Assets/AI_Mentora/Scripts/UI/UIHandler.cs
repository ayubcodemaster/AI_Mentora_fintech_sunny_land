...
namespace AIMentora
{
    ...
    public class UIHandler : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            {
                var i1 = i;
                // Use both Clickable and PointerDown for better touch/click support
                m_InventorySlots[i].AddManipulator(new Clickable(() =>
                {
                    GameManager.Instance.Player.ChangeEquipItem(i1);
                }));
                m_InventorySlots[i].RegisterCallback<PointerDownEvent>(evt =>
                {
                    GameManager.Instance.Player.ChangeEquipItem(i1);
                });
            }
            ...
            // Hide timer and coins UI
            var timerContainer = m_Document.rootVisualElement.Q<VisualElement>("TimerContainer");
            if (timerContainer != null) timerContainer.style.display = DisplayStyle.None;
            var moneyCounter = m_Document.rootVisualElement.Q<VisualElement>("MoneyCounter");
            if (moneyCounter != null) moneyCounter.style.display = DisplayStyle.None;

            // Hide settings menu button
            var settingsMenu = m_Document.rootVisualElement.Q<VisualElement>("SettingsMenu");
        ...
        public static void OpenDairyShop()
        {
            s_Instance.OpenShop_Internal("Dairy Shop", GameManager.Instance.DairyEntries);
            GameManager.Instance.Pause();
        }

        public static void OpenMeatShop()
        {
            s_Instance.OpenShop_Internal("Meat Shop", GameManager.Instance.MeatEntries);
            GameManager.Instance.Pause();
        }
        ...
        private void OpenMarket_Internal()
        {
            ...
            // Show buy/sell buttons for normal market
            m_BuyButton.style.display = DisplayStyle.Flex;
            m_SellButton.style.display = DisplayStyle.Flex;
        ...
        private void OpenShop_Internal(string shopName, Item[] items)
        {
            m_MarketPopup.visible = true;

            // Hide buy/sell buttons for view-only shops
            m_BuyButton.style.display = DisplayStyle.None;
            m_SellButton.style.display = DisplayStyle.None;
