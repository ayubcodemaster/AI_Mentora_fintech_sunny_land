...
namespace HappyHarvest
{
    public class PlayerController : MonoBehaviour
    {
        ...
        private Vector2 m_TouchMoveInput;
        private bool m_UseTouchInput;

        // Swipe detection for item switching
        private Vector2 m_SwipeStartPos;
        private bool m_IsSwiping;
        private const float SwipeThreshold = 50f; // minimum pixels to register as swipe
        ...
        private void Update()
        {
            ...
            if (Touchscreen.current != null && Touchscreen.current.primaryTouch.press.isPressed)
            {
                ...
                m_IsOverUI = m_IsOverUI || EventSystem.current.IsPointerOverGameObject(touchId);
            }

            if (!IsInputOverGameWindow())
            {
            ...
            m_CurrentWorldMousePos = GetPointerWorldPosition();

            // Handle touch-to-use on mobile (tap on ground to use tool)
            // Also detect horizontal swipes on the inventory bar area to switch items
            bool touchJustStarted = false;
            if (Touchscreen.current != null)
            {
                var primaryTouch = Touchscreen.current.primaryTouch;
                var touchPhase = primaryTouch.phase.ReadValue();
                var touchPos = primaryTouch.position.ReadValue();

                if (touchPhase == UnityEngine.InputSystem.TouchPhase.Began)
                {
                    touchJustStarted = true;
                    // Start swipe detection if touch is in bottom part of screen (inventory area)
                    if (touchPos.y < Screen.height * 0.2f)
                    {
                        m_SwipeStartPos = touchPos;
                        m_IsSwiping = true;
                    }
                }
                else if (touchPhase == UnityEngine.InputSystem.TouchPhase.Ended && m_IsSwiping)
                {
                    float swipeDelta = touchPos.x - m_SwipeStartPos.x;
                    if (Mathf.Abs(swipeDelta) > SwipeThreshold)
                    {
                        if (swipeDelta > 0)
                            m_Inventory.EquipPrev(); // Swipe right = previous item
                        else
                            m_Inventory.EquipNext(); // Swipe left = next item
                        UIHandler.UpdateInventory(m_Inventory);
                    }
                    m_IsSwiping = false;
                }
            ...
            for (int i = 0; i < overlapCount; ++i)
            {
                ...
                if (obj != null)
                {
                    ...
                    // Touch to interact with objects
                    if (touchJustStarted)
                    {
                        obj.InteractedWith();
                    }
            ...
            if (grid != null)
            {
                    ...
                    && m_Inventory.EquippedItem.CanUse(m_CurrentTarget))
                {
                    ...
                    // Touch to use tool on valid target
                    if (touchJustStarted)
                    {
                        UseItem();
                    }
            ...
            else if (Keyboard.current != null && Keyboard.current.f9Key.wasPressedThisFrame)
            {
                SaveSystem.Load();
            }
        }
        ...
        void FixedUpdate()
        {
            // Combine keyboard/gamepad input with touch joystick input
            var move = m_MoveAction.ReadValue<Vector2>();
            if (m_UseTouchInput && m_TouchMoveInput != Vector2.zero)
            {
                move = m_TouchMoveInput;
            }
        ...
        // Called by MobileJoystick UI component
        public void SetMobileMovement(Vector2 movement)
        {
            m_TouchMoveInput = movement;
            m_UseTouchInput = true;
        }

        // Called by mobile UI button
        public void OnMobileUseButton()
        {
            // For mobile button, we skip the UI check since we explicitly want to use from UI
            if (m_CurrentInteractiveTarget != null)
            {
                m_CurrentInteractiveTarget.InteractedWith();
                return;
